...
/* -------------------- XDP entry -------------------- */
SEC("xdp")
int latency_xdp_prog(struct xdp_md *ctx)
{
    ...
    if (parse_tcp(data, data_end, &iph, &tcph) < 0)
        return XDP_PASS;   /* không phải TCP */
    ...
    /* Kiểm tra hướng */
    if (iph->saddr == bpf_get_smp_processor_id()) {
        /* Không dùng bpf_get_smp_processor_id() – dùng IP máy tính.
         * Để đơn giản, chúng ta chỉ xét gói DATA khi src IP == IP máy
         * và ACK khi dst IP == IP máy. Bạn có thể thay bằng địa chỉ thực.
         */
    }

    /* Hướng DATA: máy → thiết bị */
 ...
 * Nếu muốn gửi thống kê mỗi giây từ trong eBPF, chúng ta cần một
 * BPF timer (kernel >= 5.8). Để đơn giản, ở đây chúng ta để người dùng
 * tự tính thống kê dựa trên các sự kiện đã nhận.
 * ------------------------------------------------------------------ */
