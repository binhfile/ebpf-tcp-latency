...
#define TCP_PAYLOAD_MAX_SIZE (8192)
#define TCP_PAYLOAD_SEARCH_MAX_SIZE (128)
...
struct {
    ...
    __type(key, __u32);
    __type(value, char[TCP_PAYLOAD_MAX_SIZE]);  // Increased to handle larger payloads
...
volatile const char search_pattern[PATTERN_MAX_SIZE] = {};
                                         ...
                                         const char *pattern, __u32 pattern_len)
{
    ...
    #pragma unroll
    for (__u32 i = 0; i < TCP_PAYLOAD_SEARCH_MAX_SIZE; i++) {
        if (i + pattern_len > buf_len)
            break;

        int match = 1;
        #pragma unroll
        for (__u32 j = 0; j < PATTERN_MAX_SIZE; j++) {
            if (j >= pattern_len)
                break;
...
int latency_tc_egress(struct __sk_buff *skb)
{
    ...
    if (target_ip == 0 || iph->daddr == target_ip_be) {
        ...
        __u32 bytes_to_read = payload_len > TCP_PAYLOAD_MAX_SIZE ? TCP_PAYLOAD_MAX_SIZE : payload_len;
        ...
        if (search_pattern_len > 0 && search_pattern_len <= PATTERN_MAX_SIZE) {
            if (find_pattern(buf, bytes_to_read, search_pattern, search_pattern_len)) {
                ...
                if (ev) {
                    ...
                    // Copy payload as preview
                    #pragma unroll
                    for (__u32 i = 0; i < TCP_PAYLOAD_REPORT_SIZE; i++) {
                        if( i >= bytes_to_read) {
                            break;
                        }
                        ev->payload[i] = buf[i];
